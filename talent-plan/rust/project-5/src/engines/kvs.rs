...
impl<P: ThreadPool> KvStore<P> {
    ...
    ///
    /// `concurrency` specifies how many threads at most can read the database
    /// at the same time.
    ///
    /// # Errors
    ///
...
impl KvStoreReader {
    ...
    ///
    /// `safe_point` is updated to the latest compactioni gen after a compaction
    /// finishes. The compaction generation contains the sum of all operations
    /// before it and the in-memory index contains no entries with generation
    /// number less than safe_point. So we can safely close those file handles
    /// and the stale files can be deleted.
    ...
    // Read the log file at the given `CommandPos` and deserialize it to
    // `Command`.
...
struct KvStoreWriter {
    ...
    current_gen: u64,
    // the number of bytes representing "stale" commands that could be deleted
    // during a compaction
    uncompacted: u64,
    path: Arc<PathBuf>,
    index: Arc<SkipMap<String, CommandPos>>,
...
impl KvStoreWriter {
    ...
    fn remove(&mut self, key: String) -> Result<()> {
        if self.index.contains_key(&key) {
            ...
            if let Command::Remove { key } = cmd {
                ...
                // compaction, so we add its length to `uncompacted`
    ...
    fn compact(&mut self) -> Result<()> {
        ...
        // Note that actually these files are not deleted immediately because
        // `KvStoreReader`s still keep open file handles. On Unix, the files
        // will be deleted after all the handles are closed. On Windows, the
        // deletions below will fail and stale files are expected to be deleted
        // in the next compaction.
