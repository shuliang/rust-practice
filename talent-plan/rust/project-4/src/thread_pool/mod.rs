...
pub trait ThreadPool {
    ...
    /// Spawns a funtion into the thread pool.
    ///
    /// Spawning always succeeds, but if the function panics the threadpool
    /// continues to operate with the same number of threads; the thread count is
    /// not reduced nor is the thread pool destroyed, corrupted or invalidated.
